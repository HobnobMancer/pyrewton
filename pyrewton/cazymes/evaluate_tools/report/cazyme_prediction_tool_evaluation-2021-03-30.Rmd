...
binary_stat_df <- read.csv("binary_classification_evaluation_2021_03_31.csv")
```
...
subset.spec <- binary_stat_df[which(binary_stat_df$Statistic.parameter == "Specificity"), ]
subset.spec$Prediction.tool <- factor(subset.spec$Prediction.tool, levels = c('dbCAN','HMMER','Hotpep','DIAMOND','CUPP','eCAMI')) # set order data is presented
...
Recall (also known as sensitivity) is the proportion of known CAZymes that are correctly identified as CAZymes. Figure \@ref(fig:recallbc) graphically represents of the results calculated in table \@ref(tab:sumstats).
...
subset.recall <- binary_stat_df[which(binary_stat_df$Statistic.parameter == "Recall"), ]
subset.recall$Prediction.tool <- factor(subset.recall$Prediction.tool, levels = c('dbCAN','HMMER','Hotpep','DIAMOND','CUPP','eCAMI')) # set order data is presented

# pdf(file = "binaryRecallSens.pdf", width = 8.58, height = 5.5)
p.recall = ggplot(subset.recall %>% group_by(Prediction.tool), aes(x=Prediction.tool, y=Statistic.value, fill=Prediction.tool)) +
  ...
  theme(legend.position = "none",
        ...
        axis.title=element_text(size=12,face="bold")) +
  xlab("Classifier") + 
  ylab("Recall") +  # or change to sensitivity depending on preferencer
  scale_y_continuous(breaks = seq(0.2,1, by = 0.1))
p.recall
# dev.off()
```

DIAMOND demonstrated the best performance at correctly identifying known CAZymes as CAZymes, closely followed by dbCAN.  Hotpep showed the weakest performance. The mean recall across all test sets for eCAMI (0.8580 to 4 d.p.) was greater than mean recall of CUPP (0.8541 to 4 d.p.). However, the standard deviation of recall scores of eCAMI was greater than CUPP. Therefore, eCAMI potentially has a higher probability of correctly identifying a known CAZyme as a CAZyme, but the range in performance is less consistent than CUPP. Therefore, eCAMI could generate a significantly worse performance than CUPP if used for the identification of CAZymes in a proteome, with a recall as low as 0.8 from eCAMI, meaning approximately 20% of CAZymes could be misclassified as non-CAZymes and retrieving the complete CAZome is unlikely. The smaller standard deviation of CUPP means a fixed confidence score can be assigned to CAZyme predictions by the classifier, identifying the confidence that the CAZyme predictions are correct.

The recall scores from all classifiers infers that it is unlikely the predictions tools will identify the complete CAZome from a candidate species, although the tools will identify the majority of CAZymes within the CAZyome. dbCAN and DIAMOND will identify at least 90% of CAZymes within the CAZome, although eCAMI has a tendency to identify approximately 80% of CAZymes within the CAZyome.
...
subset.prec <- binary_stat_df[which(binary_stat_df$Statistic.parameter == "Precision"), ]
subset.prec$Prediction.tool <- factor(subset.prec$Prediction.tool, levels = c('dbCAN','HMMER','Hotpep','DIAMOND','CUPP','eCAMI')) # set order data is presented
...
subset.f1 <- binary_stat_df[which(binary_stat_df$Statistic.parameter == "F1-score"), ]
subset.f1$Prediction.tool <- factor(subset.f1$Prediction.tool, levels = c('dbCAN','HMMER','Hotpep','DIAMOND','CUPP','eCAMI')) # set order data is presented
...
dbCAN and DIAMOND showed the best performances for CAZyme/non-CAZyme predictions. Hotpep, which is included within dbCAN, demonstrated the weakest performance. Again, the mean F1-score of eCAMI was greater than that of CUPP. However, the greater range in eCAMI F1-scores and standard deviation than CUPP infers a variation in performance from eCAMI than CUPP, and a user may obtain a better performance from eCAMI but that also have a good probability of obtaining a worse performance. The relatively more consistence performance of CUPP allows the user to assign CAZyme/non-CAZyme predictions an approximate confidence level.
...
bootstrap.results$Prediction_tool <- factor(bootstrap.results$Prediction_tool, levels = c('dbCAN','HMMER','Hotpep','DIAMOND','CUPP','eCAMI')) # set order data is presented

# This plot is incase you do not want the total count of each bin printed above it
# pdf(file = "binaryBootstrapAcc.pdf", width = 10, height = 6)
p.bs = ggplot(bootstrap.results %>% group_by(Genomic_accession), aes(x=accuracy, fill=Genomic_accession)) +
...
# pdf(file = "binaryBootstrapAccNumbered.pdf", width = 8.25, height = 11)
# svg(file = "binaryBootstrapAccNumbered.svg", width = 8.25, height = 11)
...
All classifiers demonstrated a bimodal shaped histogram, skewed towards the right (an accuracy of 1).

The majority of accuracies of bootstrap samples from dbCAN fell within its second biomodal peak (from 0.96-1.00), and it's lower bimodal peak ranged from 0.85-0.9. HMMER also had the majority of bootstrap accuracies fell within the upper modal peak, although this covered a greater range than the upper peak of dbCAN (0.9-1.00). However, fewer accuracies fell within the lower modal peak of HMMER (0.8 - 0.9), and predominately came from a single test set (GCA_900635675.1), which also made up half the accuracies within the lower modal peak from dbCAN. Similar to dbCAN, the lower modal peak of DIAMOND was composed mainly of bootstrap samples from GCA_001592805.2 and GCA_900635675.1, ranging from 0.75-0.90. Bootstrap samples from the remaining test sets all fell within 0.945 - 1.00.

All three of the k-mer methods did not show a reduced performance of bootstrap samples from GCA_900635675.1 to the same degree as dbCAN, HMMER and DIAMOND. Hotpep and CUPP showed similar biomodal histogram patterns (0.85-0.93, and 0.95-0.98, and overall range in accuracies was similar between the two tools, although Hotpep had more calculated accuracies within 0.9-0.87 then CUPP. This infers that in general CUPP out performs Hotpep. eCAMI showed the weakest bimodal histogram pattern. The majoirty of bootstrap samples calculated accuracies fell within 0.9-1. A much greater proportion of accuracies fell within the range of 0.9-1.0 for eCAMI than Hotpep, infering eCAMI generally produces a better performance than Hotpep. eCAMI and Hotpep had comparable number of calculated accuracies within the range of 0.8-0.9 (234 and 236, respectively), and was significantly more accuracies than CUPP, with 163 accuracies within the range. This inferred that a user has a higher probability of achieving a performance with an accuracy within 0.9-1.0 when using CUPP than using Hotpep or eCAMI.

Comparing the performances of classifiers on specific test sets is difficult for this data set. In the version of `pyrewton` used to generate the bootstrap samples, new bootstrap samples were created from each test set for each classifier, which could have lead to some of the variation in performances across classifiers on the same test set. However, the size of each bootstrap sample was the same size as the test set (n = 200), therefore, taking the performances across all bootstrap samples for each classifier (n = 600) \@ref(fig:bsacc) does provide an approximation of the expected range of performance of each classifier. This has been corrected in newer versions of `pyrewton` so that the proteins sample for each of the 100 boostrap samples is the same for each classifier. This means there will still be 100 bootstrap samples were test set, but the protein consumption of the 100 boostrap samples for one classifier will be the same for all other classifiers. This will allow for a more accurate comparison between the performances of each of the classifiers on a specific test set. This is supported by observing the same general trend if accuracy score distributions in figures \@ref(fig:bsacc) and \@ref(fig:accbc).
...
# load in the data
class_ri_ari_raw_df <- read.csv("class_predictions_classifications_2021_04_23.csv")
...
```{r}
# load in the data
cazy_class_long_df <- read.csv("class_stats_per_test_set_time_stamp.csv")
...
# merge dataframes
cazy_class_spec_recall_df <- merge(cazy_class_specificity_df, cazy_class_recall_df, by=c("Genomic_accession", "Prediction_tool", "CAZy_class"))
...
one per class, specificity vs sensitivity, one plot is one test set
...
p.class.gh.spec.recall = ggplot(cazy_class_GH_spec_recall_df %>% group_by(Prediction_tool), aes(x=Recall, y=Specificity)) +
